...
Data:
- ATP match stats from GitHub (tennis_atp): atp_matches_YYYY.csv
- MCP style clusters from GitHub (MatchChartingProject)

Models:
- PoissonRegression: counts (created, faced)
- LogisticRegression: probabilities (convert | created, save | faced) via capped Bernoulli expansion
...
def build_player_style_clusters(cfg: Config) -> Dict[str, str]:
    """
    Returns player_style_map {player_name -> style_label}
    (same features + clustering idea as your serve strategy script).
    """
    ...
    # Pressure serve/return features
...
# %% 2) Loading ATP matches from GitHub
...
def load_atp_matches_github(cfg: Config) -> pd.DataFrame:
    ...
    # ensure match_uid components exist
    if "tourney_id" not in matches.columns or "match_num" not in matches.columns:
        # fallback: create a synthetic id (still unique)
...
def add_rolling_features(pm: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    ...
    # requiring enough history
    pm = pm[pm["r_matches_played"].notna()].copy()
    pm = pm[pm["r_matches_played"] >= cfg.MIN_HISTORY_MATCHES].copy()

    # Create a lookup table of each player's rolling features per match_uid,
    # then merge onto the opponent row using (match_uid, opponent_name).
    opp_cols = [
...
def align_features_for_prediction(X_one: pd.DataFrame, models: TrainedBPModels) -> pd.DataFrame:
    """
    Force prediction row to have EXACT same columns (names + order)
...
def align_features_for_prediction(X_one: pd.DataFrame, models) -> pd.DataFrame:
    """
    Force the prediction dataframe to have EXACT same columns (names + order)
    as used during training. This is done to eliminate the sklearn 'feature names do not match' errors.
    """
    expected = models.numeric_cols + models.categorical_cols
...
def debug_schema(X_one: pd.DataFrame, models, label: str = "X") -> None:
    """
    Print helpful debug info: missing/extra columns vs training schema.
    """
    expected = set(models.numeric_cols + models.categorical_cols)
    got = set(X_one.columns.tolist())
...
def main():
    ...
    # Debug + align + predict
    debug_schema(X2, models, label="X2 (before align)")
    X2 = align_features_for_prediction(X2, models)
    debug_schema(X2, models, label="X2 (after align)")
